...
def softmax(x,axis):
    return np.divide(np.exp(x),np.reshape(np.sum(np.exp(x),1),[len(np.sum(np.exp(x),1)),1]))
...
p=90 # p is the train-validation splitting factor 
...
def DMN_Plus_Model(tf_facts,tf_question):
    ...
    #t or timsteps for the following GRU is one since concat only one
    #(word_vec_dim+hidden_szie) dimensional input will be fed. 
    #We will not be iterating through multiple words- 
    #creating hidden states in the context of previous words.
    #However we are already running the whole GRU function 
    #under a specified timestep (tf.constant timesteps), 
    #and in each timestep new input is created and previous aprev is used 
    #as the new initial aprev
...
def nearest_neighbour(x,y): 
    ...
    #Returns the tensor in x which is closest 
