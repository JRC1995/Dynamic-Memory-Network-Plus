...
unk = np.random.rand(word_vec_dim)
...
answer_index = tf.placeholder(tf.int32)
...
def DMN_Plus_Model(tf_facts,tf_question):
    ...
    def bodyf(i,hprev,srf):
        ...
        hprev = GRU(inp,hprev,wzimf,uzimf,bzimf,wrimf,urimf,brimf,wimf,uimf,bhimf,tf_seq_len,word_vec_dim)
    ...
    def bodyb(i,hprev,srb):
        ...
        hprev = GRU(inp,hprev,wzimb,uzimb,bzimb,wrimb,urimb,brimb,wimb,uimb,bhimb,tf_seq_len,word_vec_dim)
        # seq_len is the timestep for the GRU. Seq_len = no. of words in fact_no. i. 
    ...
    qr = GRU(tf_question,hprev,wzqm,uzqm,bzqm,wrqm,urqm,brqm,wqm,uqm,bhqm,tf_seq_len,word_vec_dim)
    ...
    urattm = tf.Variable(np.eye(hidden_size),dtype=tf.float32)
    wattm = tf.Variable(tf.truncated_normal(shape=[hidden_size,hidden_size],stddev=5e-2))
    uattm = tf.Variable(np.eye(hidden_size),dtype=tf.float32)
    ...
    wa1 = tf.Variable(tf.truncated_normal(shape=[hidden_size,len(vocab)],stddev=5e-2))
 
    #answer module GRU 

    y = tf.matmul(mprev,wa1) #single word answer prediction 
    #returns list of probabilities for each words in vocabulary
    return y
...
#answer = tf.constant(answer_index)
answer = tf.reshape(answer_index,[1])

# Define loss and optimizer
cost = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=model_output, labels=answer))

global_step = tf.Variable(0)
optimizer = tf.train.RMSPropOptimizer(learning_rate=learning_rate,centered=True).minimize(cost,global_step=global_step)

#Evaluate model
correct_pred = tf.reduce_all(tf.equal(tf.cast(tf.argmax(model_output,1),tf.int32),answer_index))
...
with tf.Session() as sess: # Start Tensorflow Session
    ...
    while step <= training_iters:
        ...
        for i in xrange(0,len(train_pad_facts)):
            ...
            train_answer_index = vocab.index(vec2word(train_answer[i]))
            ...
            _,loss,acc,pred = sess.run([optimizer,cost,accuracy,prediction],feed_dict={tf_fact: train_pad_facts[i], 
                                                                       ...
                                                                       answer_index: train_answer_index,
                                                                       tf_seq_len: seq_len})
            ...
            if i%100 == 0:
                print "Iter "+str(i)+", Loss= "+"{:.3f}".format(loss)+", Predicted Answer= "+str(vocab[int(pred)])+", Actual Answer= "+str(vec2word(train_answer[i]))
        ...
        for i in xrange(0,len(val_pad_facts)):
            val_answer_index = vocab.index(vec2word(val_answer[i]))
